...
const XPATH_TEMPLATE = "//file[contains(@name, '%s')]/metrics";
const CHICTREND_FOLDER = 'chictrend';
const STATUS_OK = 0;
...
function loadMetrics(string $file, string $folder): array
{
    $xml = new SimpleXMLElement(file_get_contents($file));
    $xpath = sprintf(XPATH_TEMPLATE, $folder);
...
$folder = count($argv) > 3 && $lastArgument !== '--only-percentage' ? $lastArgument : CHICTREND_FOLDER;
...
foreach (loadMetrics($inputFile, $folder) as $metric) {
